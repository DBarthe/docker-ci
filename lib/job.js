...
export const run = (version, latest=false, publish=false, onProgress) => {
  ...
  const log = ({ stdout, stderr, code }, { ignoreErrors=false } = {}) => {
    let outputChuck = "";
    if (stdout) outputChuck += stdout.trim();
    if (stderr) outputChuck += stderr.trim();
    output += outputChuck;

    onProgress(outputChuck.split("\n").map(s => s.trim()).filter(s => s !== ""));
